...
function uid(length) {
	var charlist = "abcdefghijklmnopqrstuvwzyxABCDEFGHIJKLMNOPQRSTUVWXYZ";
...
var StepContainer = function (_Component) {
	...
	function StepContainer(props) {
		...
		return _possibleConstructorReturn(this, (StepContainer.__proto__ || Object.getPrototypeOf(StepContainer)).call(this, props));
	}
...
function _controlWithProps(key, callback, validationFunc, targetIndex, disabled, label, props) {
	return _react2.default.createElement(
		...
		{ key: key, onClick: function onClick() {
				return callback(validationFunc, targetIndex, props);
			}, disabled: disabled },
		label
	);
}

var _applyControls = function _applyControls(self, props) {
	var i = void 0,
	...
	for (i = 0; i < len; i++) {
		...
		switch (control.type) {
			case 0:
				_control = _controlWithProps((0, _utils.uid)(8), _processAction, control.validationFunc, props.index - 1, false, control.label, self.props);
				break;
			case 1:
				_control = _controlWithProps((0, _utils.uid)(8), _processAction, control.validationFunc, props.index + 1, props.disableNext, control.label, self.props);
				break;
			case 2:
				_control = _controlWithProps((0, _utils.uid)(8), _processAction, control.validationFunc, undefined, props.disableNext, control.label, self.props);
				break;
		}
...
var _processAction = function _processAction(validationFunc, indexToGoto, props) {
...
var ConditionalRenderWrapper = function (_PureComponent) {
	...
	function ConditionalRenderWrapper(props) {
		...
		_this.wizardKey = (0, _utils.uid)(24);
		return _this;
	}
				...
				return _react2.default.createElement(
					_reactWizard2.default,
					{
						ref: function ref(wizard) {
							return WIZARDS = _extends({}, WIZARDS, _defineProperty({}, _this2.wizardKey, wizard));
						},
						id: id,
...
function _rehydrateState() {
	...
	this.setState(_hydrateState(this.props.hydrateState, this.props.children), function () {
		_reinitialize.call(_this3);
	});
}
...
function _initialize(reinitialization) {
	...
	this.childrenWithControls = (0, _utils.updateChildProps)(this.props.children, function (child, index) {
		return {
			index: index,
			_onPrevFunc: _this4.prev,
			_onNextFunc: _this4.next,
			_onCompleteFunc: _this4.onComplete,
			controls: _createControls(child.props, index, _this4.props.children.length - 1, _this4.props.onCompleteFunc),
			current: _this4.state.current,
			complete: _this4.state.steps[index].complete,
			warning: _this4.state.steps[index].warning,
			error: _this4.state.steps[index].error,
			disableNext: _this4.state.steps[index].disableNext
		};
	});
...
function _renderChildren(rebuildControls) {
	...
	return (0, _utils.updateChildProps)(this.childrenWithControls, function (child, index) {
		return {
			...
			complete: _this5.state.steps[index].complete,
			warning: _this5.state.steps[index].warning,
			error: _this5.state.steps[index].error,
			disableNext: _this5.state.steps[index].disableNext,
